...
// Regra exclusiva da CRIAÇÃO: a data não pode ter mais de 7 dias no passado.
...
const exigeCriacaoRecente = (d: DespesaCampos) => {
  try {
    ...
    const seteDiasAtras = startOfDay(subDays(new Date(), 7));
    return !isBefore(target, seteDiasAtras);
  } catch {
    return false;
  }
};
const REFINE_DATA_RECENTE = {
  path: ["data"],
  message: "A data não pode ter mais de 7 dias no passado.",
};
